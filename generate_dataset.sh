...
tpch_data_dir=$(dirname `realpath $0`)/tpchdata/"$database"/sf"$scale_factor"
...
if [[ ! -d "$tpch_data_dir" ]] ; then
	...
	# if [[ "$database" == "H2" ]] ; then
	#   type perl 1>/dev/null || abort_script "perl unavailable"
	#   data_files=`ls ${tpch_data_dir}*.tbl`
...
mvn install || abort_script "Error while compiling tpchbenchmark maven project"
